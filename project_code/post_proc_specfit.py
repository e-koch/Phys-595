...
def blank_the_crap(filename, min_amp_sn=3, min_wid_sn=3):
    ...
    for i in range(11):
        ...
        good_err = np.logical_or(line_pars[1] > 0,
                                  line_pars[3] > 0)
        ...
        data_copy[i][(good_err + good_sn_amp + good_sn_wid) == 0] = 0.0
        data_copy[i+11][(good_err + good_sn_amp + good_sn_wid) == 0] = 0.0
        data_copy[i+22][(good_err + good_sn_amp + good_sn_wid) == 0] = 0.0
        data_copy[i+33][(good_err + good_sn_amp + good_sn_wid) == 0] = 0.0
